...
export default function EmailCapture() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setStatus('loading');
    trackEvent('subscribe_submit', { source: 'email_capture' });
    ...
    try {
      ...
      if (res.ok) {
        ...
        setEmail('');
        trackEvent('subscribe_success', { source: 'email_capture' });
      } else {
        setStatus('error');
        setMessage(data.error || 'Something went wrong');
        trackEvent('subscribe_error', { source: 'email_capture', reason: data.error || 'request_failed' });
      }
    } catch {
      setStatus('error');
      setMessage('Network error. Please try again.');
      trackEvent('subscribe_error', { source: 'email_capture', reason: 'network_error' });
    }
  }
